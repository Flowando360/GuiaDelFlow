...
export default async function LinksEnvioPage() {
  ...
  const [{ data: links }, { data: perfiles }] = await Promise.all([
    admin
      ...
      .select('id, correo_destino, etiqueta, creado_at')
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-4 py-10 sm:px-8">
      <div>
        <p className="text-xs font-bold uppercase tracking-widest text-flow-600">Guía del Flow · Admin</p>
        <h1 className="mt-1 font-serif text-2xl font-bold text-flow-900">Links de envío</h1>
        <p className="mt-2 text-sm text-flow-800">
          Crea links especiales para compartir en vez del registro normal — quien se registre con
          uno de estos, al terminar, manda la Guía y la Carta por correo al destinatario que
          configures acá (no al correo de la persona misma).
        </p>
      </div>
